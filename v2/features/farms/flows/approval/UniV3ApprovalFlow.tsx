...
const approvalData = (jar: JarWithData, type: ApprovalType): UniV3ApprovalData => {
  if (type === "jar" && jar.depositToken.components) {
    return {
...
const UniV3ApprovalFlow: FC<Props> = ({ jar, visible, type }) => {
  ...
  const TokenContract0 = useTokenContract(approvalConfig.tokenAddress);
  const TokenContract1 = useTokenContract(approvalConfig.tokenAddress);
